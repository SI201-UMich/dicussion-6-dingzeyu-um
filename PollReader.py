...
class PollReader():
    ...
    def build_data_dict(self):
        ...
        for i in self.raw_data[1:]:
            ...
            self.data_dict['Trump result'].append(float(separated[4].strip()))
    ...
    def likely_voter_polling_average(self):
        ...
        for i in range(len(self.data_dict['sample type'])):
            if self.data_dict['sample type'][i] == 'LV':
                harris_lv_sum += self.data_dict['Harris result'][i]
                trump_lv_sum += self.data_dict['Trump result'][i]
                lv_count += 1
        
        harris_avg = harris_lv_sum / lv_count
        trump_avg = trump_lv_sum / lv_count
